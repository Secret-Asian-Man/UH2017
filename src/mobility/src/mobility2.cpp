...
int main(int argc, char **argv) {
    ...
    thread = new sf::Thread(&TurnOffLED);
    thread->launch();
    thread->wait();
...
void mobilityStateMachine(const ros::TimerEvent&) {
	...
	//soundClass.playSound(Automode);
	    ...
	    {
		//thread = new sf::Thread(&TransformLED);
		//thread->launch();
		//thread->wait();
	    ...
	    {
		//thread = new sf::Thread(&RotateLED);
		//thread->launch();
		//thread->wait();
		soundClass.playSound(Rotate);
	    ...
	    {
		//thread = new sf::Thread(&SkidSteerLED);
		//thread->launch();
		//thread->wait();
		soundClass.playSound(SkidSteer);
		...
		if (previousState != "PICKUP")
	    	{
                    //thread = new sf::Thread(&PickUpLED);
		    //thread->launch();
		    //thread->wait();
